...
def cal_t(length_road, P):
    ...
    t_max = sum(time_road)  # 求得总时间, 做下面的仿真
    return t_max, time_road


def P_suc(t_total, time_road, P):
    dt2 = 50

    t2 = np.arange(0, t_total, dt2)
    P2 = np.zeros(len(t2))
    P3 = np.zeros(len(t2))
    t_sto = 0  # 存储时间, 来进行矩形波的划分
    t_stage = 0  #

    for k in range(len(P2)):  # 先生成方波
        t_sto = t_sto + dt2
        if t_sto < sum(time_road[:(t_stage + 1)]) + 2 * dt2:  # 加了一点点防止出各种bug
            t_stage = t_stage
        else:
            t_stage = t_stage + 1
        P2[k] = P_cri[t_stage] + 20 * random.uniform(-1, 1)  # 给第k个时间来进行赋值
        P3[k] = P_cri[t_stage]
    print(t_sto, t_stage, 'hhhh', P2)
    print(t2, 't2')
    ...
    for k in range(len(t)):  # 先生成方波
        t_sto = t_sto + dt
        if t_sto < t2[t_stage] + 2 * dt:  # 加了一点点防止出各种bug
            t_stage = t_stage
        else:
            t_stage = t_stage + 1
            print('t_stage', t_stage, 't_sto', t_sto, 'k', k)
        if t_stage == 53:  # 之后再去改这里
            t_stage = 52
        P_stot[k] = P2[t_stage]  # 给第k个时间来进行赋值
    print(t_sto, t_stage, 'hhh')
    ...
    ret = 0
    for i1 in range(1, len(t)):
        Wa1 = bate[kind] * (kind_stage[kind][2] - kind_stage[kind][0]) * (kind_stage[kind][0] - P_stot[i1]) * np.exp(
            (kind_stage[kind][1] - Wa[i1 - 1]) / tao_W) \
              / (bate[kind] * kind_stage[kind][2] - P_stot[i1])  # 这里invaild的bug是明明是一维的数组, 结果有一个二维的索引
        Wa[i1] = Wa[i1 - 1] + dt * Wa1
    if min(Wa) < -60000:
        flag = 1
    ...
    return t, P_stot, t2, P3, flag, ret
...
P_cri = np.array([560, 500, 425, 400, 493, 330, 510, 425])
# 画出length和P的关系
x, P, t, Wa = distance_P(length_road, P_cri)  # 输入一组好的功率
# 这里分别是距离, 存储距离的功率, 时间, 存储时间的Wa
# 之后进行随即叠加
dx = 500
x2 = np.arange(0, max(x), dx)
print(max(x))
P2 = np.zeros(len(x2))
x_sto = 0  # 存储时间, 来进行矩形波的划分
x_stage = 0  #
for k in range(len(P2)):  # 先生成方波
    ...
    if x_sto < sum(length_road[:(x_stage + 1)]) + 2 * dx:  # 加了一点点防止出各种bug
        ...
        x_stage = x_stage + 1
    P2[k] = P_cri[x_stage] + 30 * random.uniform(-1, 1) # 给第k个时间来进行赋值

plt.plot(x2, P2, c='#2e317c', label='$Real$ $Power$')
plt.plot(x, P, c='#cf3553', label='$Ideal$ $Power$')
plt.ylabel('$Power(W)$', size=13)
plt.xlabel('$Race$ $Length(m)$', size=13)
plt.grid()
plt.legend()
plt.savefig("D:\数学建模美赛\图片\执行渡灵敏度检验_功率变化.svg", dpi=1000)  # 这样子能输出一个像素很高的图片
plt.show()

t_total, time_road = cal_t(length_road, P_cri)

flag_sto = np.zeros(100)
ret_sto = np.zeros(100)
for i in range(100):
    t, P, t2, P2, flag, ret_sto[i] = P_suc(t_total, time_road, P_cri)
    flag_sto[i] = flag

print('flag_store', sum(flag_sto))
# plt.scatter(np.arange(1, 101, 1), ret_sto)

plt.figure(figsize=[9, 5])
for i in range(100):
    if ret_sto[i] > 0:
        plt.scatter(i, ret_sto[i], c='#2e317c')
    else:
        plt.scatter(i, ret_sto[i], c='#cf3553')

# plt.plot(t, P)
# plt.plot(t2, P2)

plt.ylabel('$Residual$ $Energy(J)$', size=13)
plt.xlabel('$Experimental$ $Points$', size=13)
plt.grid()
plt.savefig("D:\数学建模美赛\图片\执行渡灵敏度检验_实际点情况.svg", dpi=1000)  # 这样子能输出一个像素很高的图片
plt.show()
